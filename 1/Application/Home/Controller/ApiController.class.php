...
class ApiController extends BaseController {
	//增加一次推荐
	public function add_one_recommened_record($supplier_company_id){
		//此处省略检测采购商是否登录
		
		if($supplier_company_id){
			//检测是否已经存在记录
			$table=M('Recommended_record');
			$where['year']=date('Y',time());
			$where['month']=(int)date('m',time());
			$where['supplier_company_id']=$supplier_company_id;

			$records=$table->where($where)->select();

			//如果存在
			if($records[0]){
				$records[0]['count']+=1;
				$result['state']=$table->save($records[0]);
			}else{
				$data['year']=date('Y',time());
				$data['month']=(int)date('m',time());
				$data['supplier_company_id']=$supplier_company_id;
				$data['count']=1;
				$result['state']=$table->add($data);
			}
			return $result;
		}else{
			$result['state']=0;
	...
	//增加一次关注
	public function add_one_watched_record($supplier_company_id){
		//检测采购商是否登录
		
		if($supplier_company_id){
			//检测是否已经存在记录
			$table=M('Watched_record');
			$where['year']=date('Y',time());
			$where['month']=(int)date('m',time());
			$where['supplier_company_id']=$supplier_company_id;
	...
	public function test(){
		$this->add_one_watched_record($_GET['id']);
	}
